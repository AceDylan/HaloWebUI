...
{#if runs.length > 0}
	{#if compact}
		<Tooltip content={`${$i18n.t('Running')} · ${runs.length}`}>
			<button
				...
				aria-label={$i18n.t('Running')}
			>
				<Bolt className="size-5 text-emerald-600 dark:text-emerald-400" strokeWidth="2" />
				<span
					class="absolute -top-0.5 -right-0.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-emerald-500 px-1 text-[10px] font-semibold leading-none text-white"
				>
					{runs.length}
				</span>
			</button>
