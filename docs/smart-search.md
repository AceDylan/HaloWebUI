...
- `research` takes no count. The CLI's `research` asks its web-discovery chain for 5 results (`count=5` in the CLI's `service.py`), asks Exa or AnySearch for 5, and fetches at most 6 candidate URLs, so a successful `research` yields about 5 URLs whatever `N` is. With `N` ≤ 5 the adapter normally stops after `research` (measured 6 s for a Chinese news query on 2026-09-24). With `N` > 5 every query also runs `search` (exit 4 on this host, about 4 s), then `doctor` on adapters older than 6623d15 (about 8 s), then `zhipu-search --count N` and `exa-search --num-results N`; the same query measured 40 s for `N` = 8. Raise `N` above 5 only if the CLI's `search` works on the host.
- `search --extra-sources` receives `min(N, 3)`; the direct provider commands receive `min(N, 10)`.

**Concurrent Requests** (`WEB_SEARCH_CONCURRENT_REQUESTS`, `rag.web.search.concurrent_requests`, source default 10) never reaches the CLI. HaloWebUI uses it only when it downloads the returned URLs: with the default `safe_web` loader it is the size of an `asyncio.Semaphore` bounding simultaneous page downloads per query (langchain's `WebBaseLoader` names the parameter `requests_per_second` but uses it as a semaphore), and with the Tavily, Firecrawl and Playwright loaders it is a true requests-per-second interval. Keep it at 1 or more: the `safe_web` loader waits forever on a semaphore of 0. The CLI's own parallelism is fixed in its code and has no config key: `research` runs its discovery providers and candidate fetches one after another, and `search` fetches its Tavily and Firecrawl extra sources in parallel. The remaining parallelism is HaloWebUI's chat handler, which keeps at most three generated queries per turn and runs up to three of them at once, so one turn can have up to three CLI processes and 3 × Concurrent Requests page downloads in flight.
