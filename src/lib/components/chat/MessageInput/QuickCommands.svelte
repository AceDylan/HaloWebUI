<script lang="ts">
	...
	// Workspace prompts as one-click chips inside the input's tool row (next to the "+"
	// menu). A click prepends the prompt to whatever is typed, so "instruction + question"
	// is one click instead of a retyped preamble. Managed in Workspace → Prompts.
	//
